# -*- coding: utf-8 -*-
"""
CodeCanvas Pro v7.0 - الإصدار الاحترافي المتقدم
Developed by Muhammed Alaa © 2026
"""
from pywebio import start_server
from pywebio.input import input, TEXT, textarea, select, radio, NUMBER, FLOAT
from pywebio.output import (
    put_html, put_buttons, put_info, put_error, put_warning,
    clear, toast, use_scope, put_collapse, put_success
)
...
SITE_NAME = "CodeCanvas Pro — استوديو صور الأكواد"
SITE_VERSION = "7.0"
...
FONT_FAMILIES = {
    "Monospace (افتراضي)": "Consolas,Monaco,monospace",
...
PRESET_SIZES = {
    "تلقائي (حسب الكود)": {"w": 0, "h": 0},
    "Full HD (1920×1080)": {"w": 1920, "h": 1080},
    "4K (3840×2160)": {"w": 3840, "h": 2160},
    "Twitter (1200×675)": {"w": 1200, "h": 675},
    "Instagram Square (1080×1080)": {"w": 1080, "h": 1080},
    "Instagram Story (1080×1920)": {"w": 1080, "h": 1920},
    "Facebook (1200×630)": {"w": 1200, "h": 630},
    "YouTube (1280×720)": {"w": 1280, "h": 720},
    "مربع صغير (800×800)": {"w": 800, "h": 800},
}
...
def generate_svg(code, theme_name, lang_name, bg_name, template_name,
                ...
                border_color="",
                # معاملات متقدمة جديدة
                custom_bg_color="", custom_bg_c1="", custom_bg_c2="",
                font_family="Monospace (افتراضي)", font_size=14,
                card_opacity=100, title_color="", title_size=22,
                desc_color="", author_color="", watermark_color="",
                watermark_size=12, watermark_position="أسفل يمين",
                numbers_color="", custom_size_name="تلقائي (حسب الكود)",
                custom_w=0, custom_h=0, line_spacing=1.7):
    ...
    font_family_css = FONT_FAMILIES.get(font_family, FONT_FAMILIES["Monospace (افتراضي)"])

    lines = code.split('\n')
    num_lines = len(lines)
    max_len = max((len(l) for l in lines), default=1)

    font = max(8, min(int(font_size), 48))
    line_h = int(font * line_spacing)
    char_w = font * 0.62
    ...
    if custom_w > 0 and custom_h > 0:
        W = custom_w
        H = custom_h
    elif custom_size_name in PRESET_SIZES and PRESET_SIZES[custom_size_name]["w"] > 0:
        W = PRESET_SIZES[custom_size_name]["w"]
        H = PRESET_SIZES[custom_size_name]["h"]

    # ═══ الخلفية ═══
    bg_defs = ""
    bg_fill = theme["bg"]

    # خلفية مخصصة (لها أولوية)
    if custom_bg_c1 and custom_bg_c2:
        bg_defs = f'<linearGradient id="bgGrad" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="{custom_bg_c1}"/><stop offset="100%" stop-color="{custom_bg_c2}"/></linearGradient>'
        bg_fill = "url(#bgGrad)"
    elif custom_bg_color:
        bg_fill = custom_bg_color
    elif bg["type"] == "solid":
        bg_fill = bg["color"]
    elif bg["type"] == "linear":
    ...
    # ═══ بناء SVG ═══
    parts = []
    parts.append(f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}" font-family="{font_family_css}">')
    parts.append('<defs>')
    ...
    if bg["type"] == "pattern" and not (custom_bg_color or (custom_bg_c1 and custom_bg_c2)):
        parts.append(f'<rect width="{W}" height="{H}" fill="url(#bgPat)"/>')

    # ═══ العنوان ═══
    y_off = outer_pad
    t_color = title_color if title_color else theme["text"]
    if title.strip():
        parts.append(f'<text x="{W // 2}" y="{y_off + 30}" text-anchor="middle" fill="{t_color}" font-size="{title_size}" font-weight="bold" font-family="{font_family_css}">{esc(title)}</text>')
        y_off += 50

    a_color = author_color if author_color else theme["accent"]
    if author.strip() or version.strip() or show_date:
        meta = []
        if author.strip():
            meta.append(author)
        ...
        parts.append(f'<text x="{W // 2}" y="{y_off + 18}" text-anchor="middle" fill="{a_color}" font-size="13" font-family="{font_family_css}" opacity="0.9">{esc(" • ".join(meta))}</text>')
        y_off += 30
    ...
    parts.append(f'<rect x="{cx}" y="{cy}" width="{cw}" height="{ch}" rx="{radius}" fill="{theme["bg"]}" fill-opacity="{opacity}" filter="url(#shadow)"/>')

    if tpl["show_window"]:
        parts.append(f'<path d="M {cx} {cy + radius} Q {cx} {cy} {cx + radius} {cy} L {cx + cw - radius} {cy} Q {cx + cw} {cy} {cx + cw} {cy + radius} L {cx + cw} {cy + header_h} L {cx} {cy + header_h} Z" fill="{theme["window_bg"]}" fill-opacity="{opacity}"/>')
        ...
        parts.append(f'<text x="{W // 2}" y="{hcy + 5}" text-anchor="middle" fill="{theme["text"]}" font-size="13" opacity="0.75" font-family="{font_family_css}">{esc(dfn)}</text>')
        parts.append(f'<text x="{cx + cw - 20}" y="{hcy + 5}" text-anchor="end" fill="{theme["accent"]}" font-size="12" font-weight="bold" opacity="0.9" font-family="{font_family_css}">{esc(lang_name)}</text>')

    # ═══ الكود ═══
    y_start = cy + header_h + pad + font
    n_color = numbers_color if numbers_color else theme["comment"]
    for idx, line in enumerate(lines):
        y_pos = y_start + idx * line_h
        if y_pos > cy + ch - pad // 2:
            break
        if show_numbers:
            parts.append(f'<text x="{cx + pad + 30}" y="{y_pos}" text-anchor="end" fill="{n_color}" font-size="{font}" opacity="0.5" font-family="{font_family_css}">{idx + 1}</text>')
            tx = cx + pad + 50
        else:
            tx = cx + pad + 10
    ...
    # ═══ الوصف ═══
    d_color = desc_color if desc_color else theme["text"]
    if description.strip():
        dy = cy + ch + 30
        ...
        for i, dl in enumerate(desc_lines):
            parts.append(f'<text x="{cx + 25}" y="{dy + 15 + i * (font + 6)}" fill="{d_color}" font-size="{font}" font-family="{font_family_css}" xml:space="preserve">{esc(dl)}</text>')

    # ═══ العلامة المائية (مع موضع مخصص) ═══
    if watermark.strip():
        w_color = watermark_color if watermark_color else theme["text"]
        if watermark_position == "أسفل يمين":
            wx, wy, anchor = W - 25, H - 15, "end"
        elif watermark_position == "أسفل يسار":
            wx, wy, anchor = 25, H - 15, "start"
        elif watermark_position == "أعلى يمين":
            wx, wy, anchor = W - 25, 30, "end"
        else:  # أعلى يسار
            wx, wy, anchor = 25, 30, "start"

        parts.append(f'<text x="{wx}" y="{wy}" text-anchor="{anchor}" fill="{w_color}" font-size="{watermark_size}" opacity="0.6" font-style="italic" font-family="{font_family_css}">{esc(watermark)}</text>')
    ...
    put_html(f'''<div style="background:linear-gradient(135deg,#667eea,#764ba2);padding:25px 30px;border-radius:16px;margin-bottom:24px;box-shadow:0 12px 32px rgba(102,126,234,0.3);color:white;">
        <div style="display:flex;align-items:center;gap:20px;flex-wrap:wrap;">
            ...
            <div style="flex:1;min-width:200px;">
                ...
                <p style="margin:6px 0 0 0;opacity:0.95;font-size:14px;">استوديو صور الأكواد الاحترافي</p>
            </div>
        ...
        <div style="margin-top:20px;display:flex;gap:20px;flex-wrap:wrap;font-size:13px;opacity:0.95;">
            ...
            <span style="display:inline-flex;align-items:center;gap:7px;">{icon("settings", 17, "white")} تحكم كامل</span>
            <span style="display:inline-flex;align-items:center;gap:7px;">{icon("sparkle", 17, "white")} تأثيرات متقدمة</span>
        </div>
    </div>''')
...
def main():
    ...
    # ═══ 1. الكود ═══
    section_title("code", "الكود", "#667eea", "الصق أو اكتب كودك")
...
# استدعاء الدالة
result = greet("أحمد")
print(result)''', rows=14, code={'mode': 'python'}, placeholder="الصق كودك هنا...")

    # ═══ 2. النصوص ═══
    with put_collapse("📝 النصوص والعناوين", open=True):
        title = input("🏷️ العنوان:", type=TEXT, value="", required=False, placeholder="مثال: دالة الترحيب")
        description = textarea("📄 الوصف:", rows=3, value="", placeholder="شرح موجز للكود...")

    # ═══ 3. معلومات المؤلف ═══
    with put_collapse("ℹ️ معلومات المؤلف (اختياري)"):
    ...
    # ═══ 4. التصميم الأساسي ═══
    section_title("palette", "التصميم الأساسي", "#9C27B0")
    ...
    bg_name = select("🌈 الخلفية الجاهزة:", options=list(BACKGROUNDS.keys()), value="بدون (لون الثيم)", required=True)

    # ═══ 5. تحكم متقدم (جديد!) ═══
    with put_collapse("⚙️ تحكم متقدم — حجم، خط، ألوان"):
        size_preset = select("📏 مقاس الصورة:",
                            options=list(PRESET_SIZES.keys()),
                            value="تلقائي (حسب الكود)", required=False)
        custom_w = input("📐 عرض مخصص (px) — 0 للتلقائي:",
                        type=NUMBER, value=0, required=False, min=0, max=5000)
        custom_h = input("📐 ارتفاع مخصص (px) — 0 للتلقائي:",
                        type=NUMBER, value=0, required=False, min=0, max=5000)
        font_family = select("🔤 نوع الخط:",
                            options=list(FONT_FAMILIES.keys()),
                            value="Monospace (افتراضي)", required=False)
        font_size = input("📏 حجم خط الكود:",
                         type=NUMBER, value=14, required=False, min=8, max=48)
        line_spacing = input("↕️ تباعد الأسطر:",
                            type=FLOAT, value=1.7, required=False, min=1.0, max=3.0)
        card_opacity = input("💧 شفافية البطاقة (%):",
                            type=NUMBER, value=100, required=False, min=0, max=100)

    # ═══ 6. ألوان مخصصة ═══
    with put_collapse("🎨 ألوان مخصصة"):
        custom_bg_color = input("🎨 لون خلفية مخصص (hex):",
                               type=TEXT, value="", required=False, placeholder="مثال: #1a1a2e")
        custom_bg_c1 = input("🌈 تدرج — اللون الأول:",
                            type=TEXT, value="", required=False, placeholder="#667eea")
        custom_bg_c2 = input("🌈 تدرج — اللون الثاني:",
                            type=TEXT, value="", required=False, placeholder="#764ba2")
        title_color = input("🏷️ لون العنوان:",
                           type=TEXT, value="", required=False, placeholder="اتركه فارغاً للتلقائي")
        title_size = input("📏 حجم العنوان:",
                          type=NUMBER, value=22, required=False, min=12, max=48)
        desc_color = input("📄 لون الوصف:",
                          type=TEXT, value="", required=False, placeholder="لون مخصص")
        author_color = input("👤 لون المؤلف:",
                            type=TEXT, value="", required=False, placeholder="لون مخصص")
        numbers_color = input("🔢 لون أرقام الأسطر:",
                             type=TEXT, value="", required=False, placeholder="لون مخصص")

    # ═══ 7. العلامة المائية ═══
    with put_collapse("💧 العلامة المائية"):
        watermark = input("💧 النص:", type=TEXT, value="", required=False, placeholder="@username")
        watermark_color = input("🎨 اللون:",
                               type=TEXT, value="", required=False, placeholder="لون مخصص")
        watermark_size = input("📏 الحجم:",
                              type=NUMBER, value=12, required=False, min=8, max=30)
        watermark_position = select("📍 الموضع:",
                                   options=["أسفل يمين", "أسفل يسار", "أعلى يمين", "أعلى يسار"],
                                   value="أسفل يمين", required=False)

    # ═══ 8. تأثيرات بصرية ═══
    with put_collapse("✨ تأثيرات بصرية"):
        show_glow = radio("🌟 توهج:", options=["لا", "نعم"], value="لا", required=True, inline=True)
        show_border = radio("🔲 إطار:", options=["لا", "نعم"], value="لا", required=True, inline=True)
        border_color = input("🎨 لون الإطار:",
                            type=TEXT, value="#667eea", required=False)

    # ═══ 9. تفاصيل ═══
    with put_collapse("📋 تفاصيل إضافية"):
        filename = input("📄 اسم الملف:", type=TEXT, value="", required=False, placeholder="app.py")
        show_numbers = radio("🔢 أرقام الأسطر:", options=["نعم", "لا"], value="نعم", required=True, inline=True)

    # ═══ توليد ═══
    put_html('<div style="height:24px;"></div>')
    put_buttons(
        ['🎨 توليد الصورة', '🎲 توليد عشوائي'],
        onclick=[
            lambda: do_generate(
                ...
                custom_bg_color=custom_bg_color, custom_bg_c1=custom_bg_c1,
                custom_bg_c2=custom_bg_c2, font_family=font_family,
                font_size=font_size, card_opacity=card_opacity,
                title_color=title_color, title_size=title_size,
                desc_color=desc_color, author_color=author_color,
                watermark_color=watermark_color, watermark_size=watermark_size,
                watermark_position=watermark_position,
                numbers_color=numbers_color, custom_size_name=size_preset,
                custom_w=custom_w, custom_h=custom_h, line_spacing=line_spacing,
            ),
            lambda: do_random(code, lang_name, filename, show_numbers),
        ]
    )
...
def do_random(code, lang_name, filename, show_numbers):
    """توليد بإعدادات عشوائية"""
    if not code.strip():
        toast("الرجاء إدخال كود!", color='warning')
        return

    random_theme = random.choice(list(THEMES.keys()))
    random_bg = random.choice(list(BACKGROUNDS.keys()))
    random_glow = random.choice([True, False])

    toast(f"🎲 تم اختيار: {random_theme}", color='success')

    do_generate(
        code=code, theme_name=random_theme, lang_name=lang_name,
        bg_name=random_bg, template_name="افتراضي",
        watermark="", show_numbers=show_numbers,
        filename=filename, title="", description="",
        author="", version="", show_date=False,
        show_glow=random_glow, show_border=False, border_color="",
        custom_bg_color="", custom_bg_c1="", custom_bg_c2="",
        font_family="Monospace (افتراضي)", font_size=14,
        card_opacity=100, title_color="", title_size=22,
        desc_color="", author_color="", watermark_color="",
...
def do_generate(**kwargs):
    ...
    with use_scope('preview'):
        ...
        try:
            svg_str, w, h = generate_svg(**kwargs)
        ...
        put_info(f"📐 {w}×{h} px | 🎨 {kwargs['theme_name']} | 💻 {kwargs['lang_name']}")
        ...
        png_js = f'''(function(){{var b64=window.__cc_b64;if(!b64){{alert('ولّد أولاً');return;}}{common_js}svgToCanvas(b64,window.__cc_w,window.__cc_h,null).then(function(c){{downloadCanvas(c,'image/png',1.0,'{out_name}.png');}}).catch(function(e){{alert('خطأ: '+e);}});}})();'''

        jpg_js = f'''(function(){{var b64=window.__cc_b64;if(!b64){{alert('ولّد أولاً');return;}}{common_js}svgToCanvas(b64,window.__cc_w,window.__cc_h,'#ffffff').then(function(c){{downloadCanvas(c,'image/jpeg',0.95,'{out_name}.jpg');}}).catch(function(e){{alert('خطأ: '+e);}});}})();'''

        webp_js = f'''(function(){{var b64=window.__cc_b64;if(!b64){{alert('ولّد أولاً');return;}}{common_js}svgToCanvas(b64,window.__cc_w,window.__cc_h,null).then(function(c){{downloadCanvas(c,'image/webp',0.95,'{out_name}.webp');}}).catch(function(e){{alert('خطأ: '+e);}});}})();'''

        svg_js = f'''(function(){{var b64=window.__cc_b64;if(!b64){{alert('ولّد أولاً');return;}}try{{var s=decodeURIComponent(escape(atob(b64)));var blob=new Blob([s],{{type:'image/svg+xml;charset=utf-8'}});var u=URL.createObjectURL(blob);var a=document.createElement('a');a.href=u;a.download='{out_name}.svg';a.style.display='none';document.body.appendChild(a);a.click();setTimeout(function(){{document.body.removeChild(a);URL.revokeObjectURL(u);}},100);}}catch(e){{alert('خطأ: '+e.message);}}}})();'''

        copy_svg_js = f'''(function(){{var b64=window.__cc_b64;if(!b64){{alert('ولّد أولاً');return;}}var s=decodeURIComponent(escape(atob(b64)));navigator.clipboard.writeText(s).then(function(){{alert('✅ تم نسخ كود SVG');}}).catch(function(e){{alert('فشل النسخ: '+e.message);}});}})();'''

        put_buttons(
            ['تحميل PNG', 'تحميل JPG', 'تحميل WebP', 'تحميل SVG', 'نسخ SVG', 'طباعة'],
            onclick=[
                lambda: run_js(png_js),
                lambda: run_js(jpg_js),
                lambda: run_js(webp_js),
                lambda: run_js(svg_js),
                lambda: run_js(copy_svg_js),
